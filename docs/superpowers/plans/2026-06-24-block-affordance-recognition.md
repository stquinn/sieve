...
1. **[DATA LOSS — do first] Transforming a sub-element inside a composite clobbers the parent.** Click a code block rendered *inside* an AI block / web-clip → the menu dispatches `sieve:extract` with `blockId = <parent composite id>`; if `TRANSFORM` is chosen, `ReplaceBlock(parentId)` replaces the WHOLE AI block → response lost. **Agreed fix (not yet built):** stamp `ContentEntry.Context["parentId"]` frontend-side when the source is nested in a `sieve-*` block; in `DetectExtractions`, map any `Transform → Extract` when entries carry a `parentId` (a nested sub-element is additive-only — it can never be replaced in place). Positioning falls out for free (`EXTRACT` uses `blockId=parentId` → `blockIndexAfter` → lands after the parent; parent survives). This also **dodges Task 8's split** for composites (their content is derived → extract-a-copy is correct). Add a Go test: "nested source ⇒ no in-place TRANSFORM offered." Label can stay "Convert to X"; only the mechanic goes additive.
...
5. `prose.Transform` newline TODO — code/diagram/log embed as verbatim `attrs["source"]`; newlines not yet preserved.
