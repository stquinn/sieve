...
- **Stage D node-granular redo — D-r.2+D-r.3 DONE** (2026-06-19). Native top-level nodes ARE the blocks (`blockId` global attr); `sieve-prose` container + `prose-renderer.js` + the `ProseIdentity` per-keystroke plugin retired; per-node load render + markered save round-trip verified in-app (CDP, zero console errors). Latent baseline bug (duplicate create-block on a loaded empty block) fixed via `seedBaseline`. See the D-r.2+D-r.3 checkbox for detail.
- **NEXT: Stage D — D-r.4** (mint `blockId` on first sync; handle the splitBlock attr-copy DUPLICATE-id trap; per-node create/update/delete ops end-to-end), then D-r.5 (retire prose `doc-update` fallback), D-r.6 (regression sweep). Until D-r.4 mints ids for split/new nodes, the `doc-update` fallback still bridges (an id-less native node forces fallback in `computeBlockSync`).
