...
- **NEXT:** **Stage E** (containers/columns by-value; column NodeView; RETIRE the `blockRef` node type + `[!block]` parser in `sieve/markdown_parser.go`; tree-aware traversal — bite-size just-in-time, couples to the editor-layout plan), then **Stage F** (lenses + server-side tree search/facets). Tech-debt **B-A** (frontend-minted prose ids) still open by user decision. **CLEANUP CANDIDATES** (mention before deleting): `render-exact-shadow.test.js` + `proseidentity-loop.test.js` pin retired designs; `mintProseIDs` (Go) orphaned by the `newDocBlock` factory; the `sieve-block-paste` duplicate-id (`handleSmartPaste` keeps source id) is a known pre-existing follow-up.
