...
- **NEXT: Stage D — node-granular redo.** See "Bite-sized (2026-06-19)" under Stage D. Representation is decided (any top-level node = one prose block, `blockId` attr); first build is D-r.2+D-r.3 (native nodes as blocks + per-node load render), then mint ids + per-node ops, retire the prose fallback, regression. Keep the app runnable (the `doc-update` fallback bridges until prose ids + per-node ops are solid).
...
- [ ] **D-r.2 + D-r.3 — Make top-level native nodes the blocks AND render per-node on load, TOGETHER, TDD.** Entangled — neither is observable alone (the locked load path forces one node), so land as one testable change. (a) Add `blockId` via `addGlobalAttributes` to the top-level node types; generalize per-node markdown serialization (the paired `<!--s:ID-->…<!--/s:ID-->` wrap `prose-renderer.js` does for the one container) to every top-level node; retire the `sieve-prose` `block+` container. (b) Make `block-render.js`/`mountWysiwyg` emit **one node per block** instead of one container per run. Confirm Go already splits a multi-paragraph run into N `FrontendBlock`s (`BlockDocToFrontendBlocks` + `scanProseRegion`); add a focused Go test, fix if not. vitest: a multi-paragraph load → N top-level nodes; native Enter in a paragraph → two blocks; a **list / table / blockquote each load as ONE block**; a structured block between two prose blocks is untouched. **First in-app WebKit gate here** (mounts, zero errors, Enter makes exactly one new delimited block; lists/tables behave natively and stay one block). Gate + commit.
- [ ] **D-r.4 — Mint blockId on first sync (handle the split-DUPLICATE) + per-node ops end-to-end, TDD.** A native node with **no id OR a duplicate id** needs minting — **THE key ProseMirror trap: `splitBlock` copies the node's attributes, so Enter gives the new half the SAME `blockId` as the original.** So "needs an id" = `blockId` is empty **or already seen among top-level nodes this pass**; mint **once** on first observe (idempotent, history-excluded, `isDestroyed`-guarded — NOT in `onUpdate`), so `create-block` carries a fresh id; thereafter `update-block`. Extend `computeBlockSync` to dedup/re-mint duplicates (today it only flags *empty* id → fallback). vitest: type in para → one `update-block`; **Enter → the new half is detected as a dup → re-minted → exactly one `create-block` with a NEW id (original keeps its id)**; Backspace-merge → one `delete-block`; minting convergent (runaway guard + no-loop assertion; re-mint must not itself loop). Gate + commit.
