...
- **NEXT — branch close-out (tightened 2026-06-20).** Stage E **descoped**; Stage F **descoped** (search → [`docs/design-block-search.md`](../../design-block-search.md); lenses reconciler-gated). The core pivot (Stages A–D) + consolidation **is the delivery** and is done and running. **To close this (large) branch, the delivery-critical frontier is exactly two items, one root:**
  1. **`nested-prose` / E-1** — the embed fix (multi-node embeds lose their id). Design ready (deferred `kind:nested-prose` container); decision locked. Backend-authoritative by construction, so it does NOT need B-A.
  2. **Legacy goldmark cleanup** — retire the redundant block parser now that the shape-driven codec is the single block-parsing authority (this is the exact area the segmentation work reworked). Route `FindBlockByID` (1 site, `context_provider.go`) + `ParseFirstBlock` (6 sites, code/log/diagram processors) through `DocumentCodec`, then delete `sieveBlockASTTransformer`/`sieveBlockNode`/`kindSieveBlock` + the dead `ParseAllBlocks`. **In scope:** block parsing only. **Out of scope:** the inline `[!kind]` parser (`sieveInlineParser`/`serializeInlineBlock`) — separate inline-smart-link concern, not codec-redundant.
