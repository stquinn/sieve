...
- **NEXT:** **S-A package decomposition** is the deliberate next effort (extract `sieve/block/` leaf-first after making `BlockServices` interfaces; mirror in JS) — it fixes the "wheat from chaff" sprawl for production AND tests at once. THEN **Stage E** (containers/columns by-value; column NodeView; RETIRE the `blockRef` node type + `[!block]` parser + the legacy goldmark `sieveBlockParser`/`FindBlockByID` still used off the codec path; tree-aware traversal) — Stage E also makes `column-row` a *supported* kind (registered processor) so the codec keeps it structured. Then **Stage F** (lenses + server-side tree search/facets). STILL OPEN: **B-D** (chain-active bracket on native prose via `.block-chrome-rail`), **B-G** (retire `serialisedForm` from the wire → collapse `FrontendBlock` into `SieveBlock`, needs WebKit by-eye + a JS render-from-attrs change), **B-A** (frontend-minted prose ids) by user decision, single-source-of-truth (retire `mdModeBuffer` as authoritative). **CLEANUP CANDIDATES** (mention before deleting): `render-exact-shadow.test.js` + `proseidentity-loop.test.js` pin retired designs; the `sieve-block-paste` duplicate-id (`handleSmartPaste` keeps source id) is a known pre-existing follow-up.
