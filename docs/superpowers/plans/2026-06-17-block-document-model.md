...
- **Stage D — partially done, then PIVOTED** (2026-06-18 → 2026-06-19). D.1/D.2/D.3 landed (prose-renderer `sieve-prose`, `block-render.js`, `computeBlockSync` observer, `frontend_block.go`) but with prose as **one `content:'block+'` block per prose run** → "1 doc == 1 prose block" coarseness (a whole run like `1\n\n2\n\n34\n\n5` serializes under one id). **The 2026-06-18 node-granular spec + its split/merge keymap attempt churned and were reverted** (`b069bde`). **New design (2026-06-19, locked):** [`docs/superpowers/specs/2026-06-19-node-granular-prose-blocks-design.md`](../specs/2026-06-19-node-granular-prose-blocks-design.md) — node-granular via **native ProseMirror split, NO keymap**. A prose block is a `kind:prose` **textblock** (not a `block+` container); native Enter splits it into a new sibling block. `nested-prose` (the `block+` tree) is deferred. The **full paired `<!--s:ID-->…<!--/s:ID-->` delimiters already implemented stay** (granularity changes, format does not). **D.4's keymap + `splitHandles`/`mergeHandles`/`Aliases` alias-union are CUT.**
- **NEXT: Stage D — node-granular textblock redo.** See "Bite-sized (2026-06-19)" under Stage D. First task is an empirical spike (textblock `inline*` sieve-prose vs. bare native nodes) run in the real WebKit app, since headings/lists handling under a no-keymap textblock has real options. Then bite-size the rest just-in-time. Keep the app runnable (the `doc-update` fallback bridges until prose ids + per-node ops are solid).
...
### Bite-sized (2026-06-19, node-granular textblock redo) — supersedes D.4/D.5
...
**The pivot in one line:** today a prose *run* is ONE `content:'block+'` `sieve-prose` block (coarse: `1\n\n2\n\n34\n\n5` under one id). Make each typed paragraph its **own** block by rendering prose as a **textblock that ProseMirror splits natively on Enter** — **no keymap, no `splitHandles`/`mergeHandles`/`Aliases`.**

**What is REUSED as-is (do not rewrite):**
- `prose-renderer.js` `markdownSerialize` — paired `<!--s:ID-->…<!--/s:ID-->` delimiters (open = id + aliases, close = id). **The full delimited block stays; format unchanged.**
- Go `scanProseRegion` / `ParseBlockDoc` / `SerializeBlockDocWithHandles` — paired-delimiter parse/serialize.
- `block-sync.js` `computeBlockSync` — per-node id-keyed diff emitting `create/update/delete-block` (already correct; it just needs to see *many* prose nodes instead of one).
- `editor.js` observer: `onUpdate` marks dirty + arms debounce (read-only); `syncDocument` → `computeBlockSync`; `docSyncFlush` on tab-switch/save; the triples builder (`editor.js:200`) that maps top-level nodes → `{id,kind,content,aliases}`.
- `sieve/frontend_block.go` `BlockDocToFrontendBlocks`; `ignoreMutation` guard on the prose NodeView.
...
**Verification gates.** D-r.1 is **pure vitest** (the app is locked to the big-bag node, so the representation decision is made in an isolated PM harness, not in-app). **In-app WebKitGTK gates start at D-r.2+D-r.3** (per `project_test_perf_in_wails_app`; and/or the CDP harness `/tmp/cdp_probe2.mjs` + open-note helper below): editor mounts, **zero** console errors, typing within a paragraph emits `update-block` and does **not** change the top-level block count, and **Enter creates exactly one new delimited block** on disk.

- [ ] **D-r.1 — Resolve the representation in an ISOLATED vitest PM harness (NOT the app).** The running app is locked to the big-bag prose block (the load/render path swallows a whole run into one node), so an in-place content toggle can't be observed — decide in a pure harness instead. The spec defers one choice: a prose block is **(A)** a `sieve-prose` textblock (`content:'inline*'`) or **(B)** bare native `paragraph`/`heading`/`list` nodes carrying a `blockId` global attr. Both give native Enter-split; they differ on headings/lists and code reuse (A keeps `prose-renderer`/`markdownSerialize`/the `editor.js:200` triples builder; B replaces them). **Steps:** build a real `EditorState` (the project's vitest PM setup) with a candidate top-level schema for each option; dispatch ProseMirror's `splitBlock` at a cursor inside a top-level prose node and **assert the top-level node count goes 1→2 with two prose nodes** (proves native Enter-split, no keymap); attempt to place a heading/list and record what each option allows. Write these as committed tests, then record **"D-r.1 RESULT:"** inline here (chosen option + the evidence). Gate: a decision backed by passing harness tests, not the locked app, not a guess.
- [ ] **D-r.2 + D-r.3 — Flip prose to the chosen representation AND render per-node on load, TOGETHER, TDD.** These are entangled — neither is observable alone (the locked load path forces one node), so land them as one testable change. (a) Change the prose node's content spec per D-r.1 (A: `prose-renderer.js` `block+`→`inline*`, keep `contentDOM`+`ignoreMutation`+`markdownSerialize`; B: `blockId` via `addGlobalAttributes` + a per-node marker serializer). (b) Make `block-render.js`/`mountWysiwyg` emit **one node per block** instead of one container per run. Confirm Go already splits a multi-paragraph run into N `FrontendBlock`s (`BlockDocToFrontendBlocks` + `scanProseRegion`); add a focused Go test, fix if not. vitest: a multi-paragraph load → N top-level prose nodes; native Enter → two; Shift+Enter stays one; a structured block between two prose blocks is untouched. **First in-app WebKit gate here** (mounts, zero errors, Enter makes exactly one new delimited block). Gate + commit.
- [ ] **D-r.4 — Mint blockId on first sync + per-node ops end-to-end, TDD.** A new prose node with no id is minted **once** on first observe (idempotent, history-excluded, `isDestroyed`-guarded — NOT in `onUpdate`), so `create-block` carries it; thereafter `update-block`. `computeBlockSync` already does the diff; wire/confirm minting feeds it. vitest: type in para → one `update-block`; Enter → exactly one `create-block` (new id); Backspace-merge → one `delete-block`; minting convergent (runaway guard + no-loop assertion). Gate + commit.
- [ ] **D-r.5 — Retire the prose `doc-update` fallback.** Once prose ids are minted and per-node ops cover create/update/delete, remove the prose-path `doc-update` fallback in `syncDocument` (keep it for structured-block edits + markdown mode, per `block-sync.js` scope note). vitest: a prose-only edit session emits only block-ops, never `doc-update`. Gate + commit.
- [ ] **D-r.6 — Regression sweep (real app + CDP).** Free-flow typing across paragraphs, cross-paragraph selection, copy/paste of a sieve block, tab-switch (no `nodeDOM`/`descAt` flood), reopen round-trips byte-stable with full paired delimiters, a multi-paragraph paste lands as N delimited blocks. Gate: all green in WebKitGTK.
