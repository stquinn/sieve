...
- **NEXT: Stage D — D-r.5** (retire the prose `doc-update` fallback — once granular ops cover prose create/update/delete, remove the prose-path fallback in `syncDocument`; keep it for structured edits + markdown mode), then **D-r.6** (regression sweep: free-flow typing, cross-paragraph selection, copy/paste a sieve block, tab-switch no `nodeDOM` flood, reopen byte-stable, multi-paragraph paste → N delimited blocks). Then Stage E (containers/columns; retire blockRef), Stage F (lenses + server tree search).
