...
- **NEXT:** **D-r.7** (unified identity + AI targeting — approved spec `2026-06-19-unified-block-targeting-design.md`, retires tech-debt B-B; pick up at the appropriate point), then **Stage E** (containers/columns; retire blockRef), **Stage F** (lenses + server tree search). Tech-debt **B-A** (frontend-minted prose ids) still open by user decision.
