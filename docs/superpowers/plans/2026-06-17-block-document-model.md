...
- **NEXT — branch close-out (tightened 2026-06-20).** Stage E **descoped**; Stage F **descoped** (search → [`docs/design-block-search.md`](../../design-block-search.md); lenses reconciler-gated). The core pivot (Stages A–D) + consolidation **is the delivery** and is done and running. **To close this (large) branch, the delivery-critical frontier is exactly two items, one root:**
  1. **`nested-prose` / E-1** — the embed fix (multi-node embeds lose their id). Design ready (deferred `kind:nested-prose` container); decision locked.
  2. **B-A** — backend-authoritative prose ids (token→mint→ack; frontend never mints, load never mutates). Retires E-1's root and the "roundtrips don't roundtrip" / load-mutates class. **E-1 and B-A are the same principle — backend owns identity — and should land together.**

  **NOT part of this plan / does NOT gate the close** — carried as tech debt in `docs/TECH-DEBT.md`, doable anytime (user decision 2026-06-20): **JS `static/` regroup** (S-A JS half), the **no-loose-functions backlog**, **B-D** (chain bracket), **B-G** (`serialisedForm`/`FrontendBlock`), the **data-loss root cause** (guard makes it non-fatal — parked), the residual **legacy goldmark `sieveBlockParser`/`FindBlockByID` deletes**, and the **CLEANUP CANDIDATES** (`render-exact-shadow.test.js`/`proseidentity-loop.test.js` pin retired designs; `sieve-block-paste` duplicate-id). None of these block correctness or the branch close; they ride on `main` afterwards.
