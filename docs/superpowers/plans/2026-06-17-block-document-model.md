...
- **NEXT:** **ShadowDoc uniform-block refactor** (tech-debt **B-C**, spec `docs/superpowers/specs/2026-06-19-shadowdoc-uniform-block-refactor.md`) — `getBlock(id)` sole accessor, retire the `Blocks` map + `syncBlocksView`, kill the `Markdown` attribute (derive on demand), rename `BlockDoc`/`DocBlock`, typed attr accessors. User-locked principle: "everything is a block, addressed by id; kind only at render/serialise." Bite-size via writing-plans; no big-bang. Fold in **B-D** (chain-active bracket on native prose via the shared `.block-chrome-rail`). THEN **Stage E** (containers/columns by-value; column NodeView; RETIRE the `blockRef` node type + `[!block]` parser; tree-aware traversal), then **Stage F** (lenses + server-side tree search/facets). Tech-debt **B-A** (frontend-minted prose ids) still open by user decision. **CLEANUP CANDIDATES** (mention before deleting): `render-exact-shadow.test.js` + `proseidentity-loop.test.js` pin retired designs; `mintProseIDs` (Go) orphaned by the `newDocBlock` factory; the `sieve-block-paste` duplicate-id (`handleSmartPaste` keeps source id) is a known pre-existing follow-up.
