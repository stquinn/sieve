...
- **2026-06-18 — Stage C-Go + Stage D.1–D.3 COMPLETE; in Stage D, D.4/D.5 pending.** Storage pivoted to the D.4 delimited-block-tree design (`2026-06-18-block-document-model-d4-design.md`). Session-fixed an editor freeze (proseIdentity deleting size-0 prose looped `EditorState.apply`), dropped StarterKit `TrailingNode` (it fabricated an unselectable empty trailing block), and added a valid id-less trailing prose editing surface — commit `6469b5f`.
- **2026-06-18 — NEW GOVERNING SPEC for remaining prose work: `2026-06-18-node-granular-blocks-design.md`.** It **supersedes the D.4 prose-granularity decision** (one multi-paragraph leaf → **many `sieve-prose` blocks, one id each**; `content:'block+'` kept for cemented rich blocks; Enter splits, cement draws one boundary). Re-scopes the rest of Stage D: **D.2** loads each shadow prose block into its own `sieve-prose` (not one concatenated block); **D.4** = Enter splits `sieve-prose` into a sibling + `splitHandles` tail mint / Backspace `mergeHandles` (Go handle math already exists from Stage B). One open mechanic to settle when planning D.4: uniform `sieve-prose`+split-on-Enter (recommended) vs bare-top-level-nodes+`sieve-prose`-for-cement. **NEXT: bite-size revised Stage D via writing-plans.**
