...
- **NEXT: Stage D — D-r.6** (regression sweep: free-flow typing, cross-paragraph selection, copy/paste a sieve block, tab-switch no `nodeDOM` flood, reopen byte-stable, multi-paragraph paste → N delimited blocks). Then Stage E (containers/columns; retire blockRef), Stage F (lenses + server tree search).
