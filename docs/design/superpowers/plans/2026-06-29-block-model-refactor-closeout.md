...
> **STATUS — ✅ COMPLETE (closed out 2026-06-30).** All tasks landed (the in-file checkboxes were tracked via the per-task SDD briefs, not ticked here): **Task 1** `handle_gc` → ShadowDocument/SieveBlock (`89b0931`); **Task 2** AIService response/filing helpers (`88081ad`); **Task 2b** `block/` codec/parser helpers → DocumentCodec/SieveBlock (`6f61a27`); **Task 3** test debt — deleted stale `render-exact-shadow` (C-T) + made the `updateNotify` debounce opt-out explicit (T-A) (`d6f6d94`); **Task 4** L-A — clear to an empty paragraph on a genuinely-empty reload (`355da02`); **Tasks 5 & 6** (meta) produced the dedicated **B-A** and **F-A** plans, both since executed + merged (B-A `8ca1f93`, F-A `b77e802`); **Task 7** S-A JS regroup mirroring the Go packages (`b8ec4ff`). Close-out branch merged via `a44e09a`. Rail/lineage/affordance UI was **explicitly out of scope** — re-planned as a Stage E/F affordance re-brainstorm (TECH-DEBT **U-A**). Residual no-loose-functions stragglers (e.g. `scanProseRegion`, goldmark helpers, `ai/eval`) remain tracked in `docs/TECH-DEBT.md` / CLAUDE.md as opportunistic cleanup, not part of this plan's scope.
