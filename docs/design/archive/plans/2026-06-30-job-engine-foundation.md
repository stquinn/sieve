...
**Architecture:** This is **Phase 0 + Phase A** of the communal-job-engine design (`docs/design/archive/specs/2026-06-30-async-ai-job-queue-design.md`). Phase 0 is pure repackaging: (1) move `block/language_heuristics.go` to a new `sieve/lang` leaf so `ai` stops importing `block` (the smell that forced `block.AIPort`); (2) move `EditorService` from `services` into a new top-level `sieve/editor` package above `ai`, so that when Phase B makes the editor instigate AI document jobs it can call `AIService` directly without an `ai ↔ services` cycle. Both are behaviour-neutral relocations. Phase A adds the generalised `JobTracker` (Active **and** Queued lists, `jobs:changed`, `/api/jobs`) and the `JobEngine` (one communal instance, a bounded worker pool per `Category`) as new, additive code in `services`. **Nothing existing is rewired or deleted here beyond the package moves** — every commit builds green. The behaviour-changing cutover (delete `block.AIPort`, declarative processors, `EditorService` document entries, retire the old tracker writers, fold in close-all, frontend) is **Plan 2 (Phase B/C)**, written after this lands.
