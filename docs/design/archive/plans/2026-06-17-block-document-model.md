...
**Spec:** [`docs/design/archive/specs/2026-06-17-block-document-model-design.md`](../specs/2026-06-17-block-document-model-design.md)
...
- **Data-loss guard — DONE (2026-06-20).** `flushShadow` refuses to overwrite a **non-empty** doc with **empty** content — an empty derive (failed `codec.Serialize` → `deriveMarkdown` returns "", or a transient empty markdown-mode buffer) was wiping files. Regression test reproduces the wipe. **Root-cause hunt PARKED (user decision 2026-06-20):** not reproducible on demand; the guard makes it non-fatal, so it no longer gates the branch. Revisit only if it recurs — capture the log then (`"serialize block doc failed"` vs the new `"REFUSED empty overwrite"`). Commit `b7dd63e`.
...
- **✅ CLOSE-OUT COMPLETE — branch ready to merge (2026-06-21).** Stage E **descoped**; Stage F **descoped** (search → [`docs/design-block-search.md`](../../design-block-search.md); lenses reconciler-gated). The core pivot (Stages A–D) + consolidation **is the delivery** and is done and running. Both delivery-critical close-out items landed, plus two more found during testing:
  1. ✅ **E-1 (embed fix) — DONE**, via the `proseGroup` **content-derived** container — NOT the `nested-prose` kind (simpler: no on-disk flag, wire kind stays `prose`, the renderer's own parse count picks native-vs-container; backend-authoritative by construction so it does NOT need B-A). User-verified in-app. Commits `77ad7ae`, `cd6c66d`. Tech-debt **E-1 resolved**.
  2. ✅ **Legacy goldmark cleanup — DONE** — `FindBlockByID`/`ParseFirstBlock` routed through `DocumentCodec`; `sieveBlockASTTransformer`/`sieveBlockNode`/`kindSieveBlock` + dead `ParseAllBlocks` deleted; inline `[!kind]` parser kept (separate concern). Commits `c9c6b73`, `177863f`.
  3. ✅ **notifySaved on every save, not just debounce** (`5272091`) — embed/AI-job/shutdown direct flushes now post `flush-ack` so the frontend clears its dirty indicator.
  4. ✅ **Version-restore corruption — root-caused + fixed** (`c508aaa`, `655d7ac`). The frontend's flat `setContent` was a SECOND, lossy document parser (couldn't read `<!--s:ID-->` markers → re-minted prose ids → persisted the corruption on save-back). Fixed by routing restore through `EditorService.ReloadFromDisk` + the block list; **the last `setContent(markdown)` doc-load is retired** — wysiwyg always renders the backend block list, only Go parses document structure from markdown. New tech-debt **F-A** (the remaining OUT-direction whole-doc serialize + fence-rules — lower-risk, deferred); **T-A**/**L-A** (flaky teardown test; empty-blocks reload edge) recorded.

  **B-A — DEFERRED to its own future branch (user decision 2026-06-20):** backend-authoritative prose ids (token→mint→ack; frontend never mints, load never mutates) is foundational but lands in the two-prior-reverts editor churn zone. `nested-prose` fixes E-1 without it; the system is correct without it (typed prose works). Keep it as carried debt; give it a focused branch later so it can churn safely.

  **NOT part of this plan / does NOT gate the close** — carried as tech debt in `docs/TECH-DEBT.md`, doable anytime (user decision 2026-06-20): **B-A** (above), **JS `static/` regroup** (S-A JS half), the **no-loose-functions backlog**, **B-D** (chain bracket), **B-G** (`serialisedForm`/`FrontendBlock`), the **data-loss root cause** (guard makes it non-fatal — parked), and the **CLEANUP CANDIDATES** (`render-exact-shadow.test.js`/`proseidentity-loop.test.js` pin retired designs; `sieve-block-paste` duplicate-id). None of these block correctness or the branch close; they ride on `main` afterwards.
...
**Spec:** [`docs/design/archive/specs/2026-06-19-node-granular-prose-blocks-design.md`](../specs/2026-06-19-node-granular-prose-blocks-design.md). **Read it before executing.**
