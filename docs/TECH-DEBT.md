...
**Retires when:** The `window.TipTap` / `window.Sieve*` global bus is replaced by explicit ES `import`/`export` modules (boundaries enforced by the import graph), and `editor.js` is decomposed into cohesive, independently-importable units. Aligns with the Go-server + web/mobile direction (a global god-object won't survive a bundler/SSR boundary). User decision 2026-06-29. **Update 2026-07-08:** the epic now exists — Forgejo #31 (Workspace/Editor component model, phases #27–#30) with normative specs (`docs/design/specs/2026-07-08-workspace-editor-component-model.md` + companion) and `docs/how-to-idiomatic-js.md`. **Update 2026-07-20 (evidence refresh, drift predates #43):** the `window.TipTap` sub-claim is RETIRED — every file now reads TipTap/PM through the single seam `frontend/src/static/base/tiptap-vendor.js` (`export const T = globalThis.TipTap || {}`, NORMATIVE per its header, P4.E D-1) *(path/doctrine updated by #49 P5, 2026-07-22: the seam moved to `lens/surfaces/tiptap-vendor.js` and split into TWO named seams over the one global — PM reads there; bundled non-PM libs via `renderers/vendor-libs.js`)*, and `editor.js` no longer exists (decomposed into `lens/abstract-editor.js` + `note-editor.js`/`prompt-editor.js` + `editor-shell.js` under #31). The `window.Sieve*` bus remains live (71 refs / 28 files, e.g. context-menu.js ×15) — X-C stays open on that basis. **Update 2026-07-20 (loose-function survey, full results on #43):** two more god-files belong to this item — `lens/context-menu.js` (top-level loose functions + one-method `window.SieveContextMenu` bus) and `ai/ai-actions.js` (`window.SieveAI` + `window.__sieveActiveJobs` IIFE; its `updateStatusBar` hand-builds half of the status bar that `shell/status-bar.js`'s StatusBar class owns the other half of). Minor: `ui/sidebar.js`, `ui/tabbar.js`, `ui/layout.js` bare IIFEs.

## X-D: Sieve block renderers are duck-typed config bags — ✅ RETIRED 2026-07-22

**Resolved by #49 P5** (commit 15549d0, protocol-extraction epic final phase). The registration/adapter half became typed: `sieve-block-extension.js`'s `export let registerSieveRenderer` + registration-IIFE-rebinding pattern replaced by a real `NodeViewRegistry` class (`#private` state), with the adapter contract an explicit JSDoc `SieveBlockAdapter`/`SieveNodeView` typedef surveyed from the actual call sites (the informal "Renderer interface" header prose superseded). The catalogued adapter-side loose functions landed on owning types: `BlockSelection` statics (`lens/block-selection.js`, the DOM-selection↔block family), `HeaderBar` statics (focus adopt/restore), `JobStatusTracker` singleton (`renderers/job-status.js` — the stateful job sets, kept OFF pure `StatusBadge.classify`), `esc` → `renderers/html-escape.js`; dead `resolveDisplaySrc`/`srcToBlockId`/`extractTextFromDOM` deleted (zero callers, verified). `base/fenced-block-base.js` — the "base in name only" — dissolved into `renderers/` the same phase (base/ now holds only the X-C pair `icons.js`/`globals.js`). No separate BlockFramework facade object was built: the registry + typed adapter contract fill that role (orchestrator judgment at retirement). **Residuals live elsewhere, not here:** `block-chrome.js` internals + `window.Sieve*` bus → X-C (epic #31); `docs/how-to-sieve-block-framework.md` rewrite still pending (banner'd STALE; `docs/how-to-intelligent-fenced-blocks.md` updated to the new import homes in this close-out). The `smart-link` renderer that was parked here is **never built** — the links decision landed 2026-07-27 (#67, `docs/design/archive/specs/2026-07-27-inline-block-removal-links-decision.md`): the kind and the whole inline mode are DELETED, so 7 of 7 registered kinds are migrated.

<details><summary>Original item (retained for context)</summary>

**Tracked:** **Forgejo #43** (block-renderer-extraction epic, phases #44–#47; spec `docs/design/archive/specs/2026-07-20-block-renderer-extraction.md`) — was register-only (kept OUT of epic #31 by user decision 2026-07-08). **Refinement over the sketch below (2026-07-20):** the base class is a PM-FREE `BlockRenderer` (attrs→DOM + carried stylesheet under the theme-vars-only contract) and the NodeView is a thin adapter HOLDING a renderer (composition) — NOT a base class owning NodeView lifecycle — so renderers serve non-PM hosts (chat lens, embedded cards). Adapter-side loose functions confirmed 2026-07-20 in `sieve-block-extension.js` (`shouldClaimBlockSelection`, `domSelection*`, `adoptFocusedControl`/`restoreFocusedControl`, `extractContentEntryFromEditor`, the registration IIFE + `export let` rebinding) and `lens/extensions.js` (`resolveDisplaySrc`/`srcToBlockId`, `buildAiContext`/`applyTargetHighlight`) stay OUTSIDE #43's scope: adapter/framework cleanup lands with this item's retirement, AI-seam pair belongs to #31/X-C.

**What:** The 8 NodeView adapters in `frontend/src/static/lens/surfaces/node-views/` (moved+renamed 2026-07-21 from `processors/*-renderer.js` — they are NodeViews, and PM enters the JS graph only in surfaces) are duck-typed config objects (`{ getIcon: function() {…}, … }`) whose required shape is documented nowhere — it is implicit in what `sieve-block-extension.js` calls, learned by reverse-engineering. `fenced-block-base.js` is a base in name only: a grab-bag of free functions (`getLowlight`, `hastToHtml`, `applyHighlighting`) imported piecemeal. There is no `BaseSieveBlockRenderer` to open and understand the family *(update 2026-07-20: closed for the look-and-feel half — `BlockRenderer` fills that role, composed not inherited; the registration/adapter half this describes remains duck-typed)* — the readability gap the JS-OOP principle (CLAUDE.md 2026-07-08) exists to close.
...
**Update 2026-07-20 (loose-function survey, full results on #43):** `lens/block-chrome.js` is a previously-uncatalogued offender of the same shape — top-level mutable module state (`dragState`, `lastSelectedOffset`) + loose functions, with its two DOM-construction strategies (`createChromeHostWidget` prose / `populateChromeHost` sieve atoms) ~90% copy-paste duplicates of each other's handle-wiring. The survey also counted 11 duplicated-behaviour clusters across the 8 renderers (~35+ duplicate implementations; worst: the job-status badge state machine ×6, already drifted — smart-link lacks the TIMEOUT branch); those are epic #43's base-class feedstock, catalogued on the epic.

**Update 2026-07-21 (contract rev 2 reconcile):** the APPROVED Block Renderer Contract (`docs/design/archive/specs/2026-07-21-block-renderer-contract.md`) landed on top of the P4 state: renderers now take the typed `SieveBlock` envelope (`contract/sieve-block.js` — raw attr maps cross no consumer signature), speak SEMANTIC VERBS (core `setMode`/`setContent`/`retry`/`expand` + subclass verbs) over private `_pushAttrs`, and effect through the `ContainerTransport`/`DocumentService` pair (`container/container-transport.js`, `container/document-service.js` — Workspace composition root; v1 appliers registered per NodeView). The `Projected*` subclasses are replaced by the `handleBuild` region interceptor; the seam authors projected bodies via scratch instances; the interaction policy reads `block.mode` (typed), never raw attrs. This RETIRES the actions-bag/attr-leak half of this item and gives the future typed registry its vocabulary.

**Retires when (updated 2026-07-20):** superseded by the composition design that actually shipped (see "Refinement" above) — PM-free `BlockRenderer` (`renderers/block-renderer.js`) + `RendererStyleRegistry` + `StatusBadge`/`LineGutter`, one concrete renderer + sibling `*.styles.js` per kind. 7 of 8 kinds migrated (diagram #45, ai-block #46, code/log/web-clip/smart-card/smart-image #47); `smart-link` deliberately parked. **What remains open:** the registration/adapter contract is still an untyped duck-typed object (`sieve-block-extension.js`'s `registerSieveRenderer`; its header still documents the old "Renderer interface" informally) — X-D fully retires when that becomes a typed registry per `docs/how-to-idiomatic-js.md`, the BlockFramework facade lands (spec §Decision), and the adapter-side loose functions from the 2026-07-20 survey are addressed. Registration via a typed registry method, JSDoc-typed contract per `docs/how-to-idiomatic-js.md`; `docs/how-to-sieve-block-framework.md` rewritten as "extend the class". Note: `smart-link-renderer` is **never built** — the links decision landed 2026-07-27 (#67, `docs/design/archive/specs/2026-07-27-inline-block-removal-links-decision.md`): `smart-link` and `BlockModeInline` are DELETED, so the eighth renderer does not exist and 7 of 7 kinds are migrated.

</details>

## V-A: "Hide AI blocks" is a frontend CSS lie, not a backend-filtered DocView projection

**What:** `toggleAiBlocks` (Mod+Alt+J) hides AI blocks with a **frontend-only CSS visibility toggle** — `#showAiBlocks` state on `AbstractEditor`, mirrored as a `hide-ai-blocks` class on the editor root (`presentSurface` syncs it; `editor.css` does `display:none`). The AI blocks are still in the document, still in the DOM, still round-trip and save — the toggle only stops *painting* them. It works, but it is not structurally correct: a document is entirely blocks, and "show me the doc without AI blocks" should change **what the editor is given**, not apply a visual mask over what it holds. The state is also editor-carried chrome that must be nursed per-instance (it caused the stale-class desync bug patched in P2.C.2).
...
## X-F: Ten lens→host chrome couplings the #96 cutover left behind

**Tracked:** register only (found by P4c's widened isolation tripwire; #96).

**What:** `lens/` may import only `contract/` + `renderers/` (plus the `ident/`/`vendor/`/`base/` leaves), and `frontend/test/lens-isolation.test.js` checks that transitively. Ten edges fail that rule today and are frozen in the test's `QUARANTINE` list so they can neither grow nor be widened away: the block/prose paths open host chrome inline (`ui/media-lightbox.js` ×3, `ui/link-edit-dialog.js` ×2, `ui/copy-image.js`), `WysiwygSurface` builds the shell's `@`-mention popover itself (`shell/trigger-host.js`, `trigger-popover.js`, `trigger-providers.js`) rather than exposing its `CaretTriggerPort` for the host to attach one, and `renderers/asset-urls.js` resolves asset URLs through `generated/protocol.js` — which drags the wire module into every lens that paints with a renderer, and is the last thing stopping a lens from extending `BlockRenderer`.
...
## F-B: the find bar re-hangs itself only on open, not on every follow

**Tracked:** register only (#61 close-out, 2026-09-03).

**What:** `FindDialog.#hang` appends the bar into `#editor-col` from `open()` alone. `#follow()` — which now runs on tab activation as well as on every verb — re-arms the wire but never re-checks which element the bar is parented to.

**Why deferred:** `#editor-col` is static markup in `index.html`: one element for the life of the page, whatever is mounted inside it. While that holds, asking once per open is asking as often as the answer can change, and the bar falls back to `document.body` where the column is absent.

**Retires when:** `#editor-col` becomes swappable — an hx-swap/oob target, or a per-tab pane — at which point the `#hang` call moves into `#follow()` so the bar is re-parented wherever the reader now is.
