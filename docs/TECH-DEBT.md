...
**Why deferred:** not a block-model regression; surfaced during verification. **Retires when:** the paste path mints a fresh id for the pasted block (clear the copied id so it routes through `create-block` / server mint, the same discipline `mintActions` uses for `splitBlock`). Moved here from the archived plan. **Folds into C-V** (the slice-paste create-op fix mints the fresh id in the same change).

## C-V: `sieve/slice` paste doesn't sync to Go — pasted blocks lost on round-trip

**What:** Pasting a copied Sieve block reconstructs it **client-side** (`editor.js` `sieve/slice` → `insertContent`) but emits no `create-block` op, and `computeBlockSync` emits nothing for a structured create — so Go's tree never learns of the block. It survives in-session but vanishes on the next Go-sourced render/reload. Latent before; **now visible** because the create convergence + removal of the `doc-update` fallback made Go's tree authoritative (the fallback's whole-doc reparse used to sync these silently).

**Why deferred (2026-06-21):** surfaced right after the create convergence; the machinery to fix it (the positioned `block-op {create-block}` path) only just landed. **Retires when:** the `sieve/slice` paste emits a `block-op {create-block, kind, attrs, index}` per pasted sieve block with a **fresh id** — the same path the toolbar create now uses (this also retires **P-D**). Prose slice items keep their client-first observer sync.
