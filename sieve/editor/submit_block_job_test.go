...
func TestSubmitBlockJob_AppliesThenFinishesOnSuccess(t *testing.T) {
	...
	if blk.Attrs["language"] != "go" {
		t.Fatalf("Apply did not mutate blk: %+v", blk.Attrs)
	}
...
func TestSubmitBlockJob_ErrorSkipsApply(t *testing.T) {
	...
	case err := <-done:
		if err == nil {
			t.Fatal("expected error to reach onDone")
		}
	case <-time.After(2 * time.Second):
		t.Fatal("onDone never called")
	}
	if applied {
		t.Fatal("Apply must NOT run on error")
	}
}
...
func TestSubmitBlockJob_NilWorkStillApplies(t *testing.T) {
	...
	if blk.Attrs["applied"] != true {
		t.Fatalf("Apply should run for nil Work")
	}
}
