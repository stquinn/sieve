...
func TestAttachments_StampAttrs(t *testing.T) {
	...
	Attachments{{URI: "container:9f2b", Title: "Auth Design"}}.StampAttrs(attrs)

	list, ok := attrs[AttachmentsAttr].([]interface{})
	if !ok || len(list) != 1 {
		t.Fatalf("attrs[%s] = %#v", AttachmentsAttr, attrs[AttachmentsAttr])
	}
	if entry := list[0].(map[string]interface{}); entry["uri"] != "container:9f2b" || entry["title"] != "Auth Design" {
	...
	Attachments{{URI: "container:9f2b"}}.StampAttrs(nil) // must not panic
}
