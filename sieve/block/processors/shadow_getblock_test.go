...
// kind only matters when turning the block into context text). GetBlock(id) on the lock-free DocView snapshot is
...
func TestShadowDocument_getBlock_resolvesProse(t *testing.T) {
	...
	if b.Kind != block.KindProse || b.Content() != "the quick brown fox" {
...
func TestBuildContextForID_resolvesProseContentFromDoc(t *testing.T) {
	...
	if got := block.BuildContextForID("pr-1", doc, map[string]bool{}); got != "the quick brown fox" {
...
func TestBuildContextForID_gathersProseRefChain(t *testing.T) {
	...
	doc := block.DocView{Blocks: []block.SieveBlock{
		...
		{ID: "pr-3", Kind: block.KindProse, Attrs: map[string]interface{}{"content": "gamma"}},
	}}
	seen := map[string]bool{}
	var got []string
	for _, id := range []string{"pr-1", "pr-2", "pr-3"} {
		if c := block.BuildContextForID(id, doc, seen); c != "" {
			got = append(got, c)
		}
	}
