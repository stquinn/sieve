...
// NewBlockAnchorProvider constructs a BlockAnchorProvider with the injected
// services. Exported so the composition root can build it (svc is unexported).
func NewBlockAnchorProvider(svc BlockServices) *BlockAnchorProvider {
	return &BlockAnchorProvider{svc: svc}
}
