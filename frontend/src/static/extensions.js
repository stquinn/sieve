...
;(function () {
  ...
  // ── getAiTargetLabel ───────────────────────────────────────────────────────
  // Read-only equivalent to buildAiContext for live UI tracking.
  // Performs the same tree traversal but strictly skips any mutations (no wrap, no ID generation).
  function getAiTargetLabel(editor, isMarkdownMode) {
    ...
    if (!aiBlockId && !targetNode) {
      for (var d = $from.depth; d >= 0; d--) {
        var n = $from.node(d)
        if (n.type.name === 'aiBlock' || n.type.name === 'sieve-ai-block') { aiBlockId = n.attrs.id || ''; break }
        if (n.type.name === 'blockRef' || n.type.name.startsWith('sieve-') || n.type.name === 'codeBlock' || n.type.name === 'table') { targetNode = n; break }
      }
    }
    ...
    if (!aiBlockId && !targetNode) {
      ...
      doc.nodesBetween(scanFrom, scanTo, function (node, pos) {
        if (!targetNode && (node.type.name === 'blockRef' || node.type.name.startsWith('sieve-') || node.type.name === 'codeBlock' || node.type.name === 'table')) {
          if (node.type.name === 'aiBlock' || node.type.name === 'sieve-ai-block') {
             aiBlockId = node.attrs.id || ''; return false
          }
          targetNode = node; return false
        }
      })
    }

    if (aiBlockId) return 'Follow-up'
    ...
    if (targetNode) return labelFor(targetNode)
