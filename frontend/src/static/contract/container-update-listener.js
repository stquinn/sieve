 ...
 *
 * IT IS ANCHORED BY NAME, NOT BY POSITION. `quote` and `occurrence` say which
 * characters are meant; `start`/`end` are offsets into the block's STORED form,
 * which is not the reading a lens draws, so they are a hint and never the
 * answer. A lens finds occurrence `occurrence` of `quote` in its own reading and
 * DROPS the mark where that does not resolve — a mark is derived from text that
 * has since moved on, and staleness is its absence.
 *
 ...
 * block's COMPLETE set and replaces what the lens held for it, so an empty array
 * is the clear and not a no-op. It is optional — a lens with nowhere to draw a
 * mark simply does not implement it — and `subscribe` cues it for every block
 * the host already holds marks for, the way it cues `onChanged` with the whole
 * container.
 *
