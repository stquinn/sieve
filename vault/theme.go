...
//  1. <vaultRoot>/themes/<name>.json  — user-overridable vault-local themes
...
func LoadTheme(vaultRoot, name string, builtins fs.FS) ThemeVars {
	if name == "" {
		name = "tokyonight"
	}

	// 1. Vault-local override takes priority
	vaultPath := filepath.Join(vaultRoot, "themes", name+".json")
	if data, err := os.ReadFile(vaultPath); err == nil {
		if vars := parseTheme(data); vars != nil {
			fmt.Printf("[stash] theme: loaded from vault: %s\n", vaultPath)
			return vars
		}
	}
