...
func (v *Store) ListHistory(uuid string) []HistorySnapshot {
	p1 := filepath.Join(v.HostHistoryDir(), uuid+".*.md")
	p2 := filepath.Join(v.StoreHistoryDir(), uuid+".*.md")
...
func (v *Store) SaveVersionSnapshot(uuid string, version int, content string) error {
	isFiled := regexp.MustCompile(`(?m)^status:\s*filed`).MatchString(content)
	dir := v.HostHistoryDir()
	if isFiled {
		dir = v.StoreHistoryDir()
	}
...
func (v *Store) PruneHistory(uuid string, maxVersions int) error {
	if maxVersions <= 0 {
		return nil
	}
	p1 := filepath.Join(v.HostHistoryDir(), uuid+".*.md")
...
func (v *Store) DeleteHistory(uuid string) error {
	p1 := filepath.Join(v.HostHistoryDir(), uuid+".*.md")
	p2 := filepath.Join(v.StoreHistoryDir(), uuid+".*.md")
	m1, _ := filepath.Glob(p1)
	m2, _ := filepath.Glob(p2)
