...
// pastes). key is the asset filename relative to the category directory,
// e.g. "assets/blk-abc123.png". The Encoding is inferred from magic bytes
// by the Store — the caller never declares it.
//
...
func (as *AssetService) Save(category store.Category, key string, data []byte) (*ImageAsset, error) {
	...
	if !ok {
		return nil, fmt.Errorf("asset: save %s: created storable is not AssetStorable", key)
	}
